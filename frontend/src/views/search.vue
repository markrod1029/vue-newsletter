  ...
  <div>
    ...
    <div class="mb-6">
      <div class="flex gap-2">
        <input class="form-input" placeholder="Search approved posts..." v-model="searchQuery"
    ...
    <div v-else>
      ...
      <div v-else class="grid grid-cols-1 gap-6">
        <div v-for="post in posts" :key="post.id" class="card hover:shadow-lg transition-shadow">
          <div class="card-body">
            ...
            <!-- Media Display - Image or Video -->
            <div v-if="post.media_url" class="media-wrapper mb-4">
              <img v-if="isImage(post.media_url)" :src="getMediaUrl(post.media_url)" :alt="post.title"
                class="media-item" @error="handleImageError">
              <video v-else-if="isVideo(post.media_url)" :src="getMediaUrl(post.media_url)" class="media-item" controls
                preload="metadata">
      ...
      <div class="mt-6 flex justify-center" v-if="pagination.last_page > 1">
        <div class="pagination">
          <button v-for="page in pagination.last_page" :key="page" class="pagination-item"
            :class="{ 'pagination-item-active': page === pagination.current_page }" @click="handlePageChange(page)">
            {{ page }}
          </button>
        </div>
      </div>
...
/* Media (image or video) fills the wrapper */
.media-item {
  position: absolute;
  top: 0;
  left: 0;
